...
app.get("/api/resolve-playwright", async (req, res) => {
  ...
  if (!embedUrl) {
    return res
      .status(400)
      .json({ error: 'Parámetro "url" requerido (?url=...)' });
  }
  ...
  try {
    ...
    if (result && result.url) {
      ...
      return res.json({
        ...
        rawUrl: result.url,
        cookies: result.cookies || "", // ← pasar al frontend
    ...
    return res
      .status(404)
      .json({ error: "No se pudo resolver el enlace .m3u8" });
  } catch (error) {
...
app.get("/api/stream", async (req, res) => {
  ...
  const cookies = req.query.cookies || ""; // o de un header
  ...
  try {
    ...
    const response = await fetch(targetUrl, {
      headers: {
        ...
        Origin: originHost,
        Cookie: cookies, // ← REENVIAR COOKIES
      },
    });
    ...
    if (!response.ok) {
      return res
        ...
        .send(`Error CDN Provider: ${response.status}`);
    }
  ...
  try {
    ...
    let extractedMediaUrl = await page.evaluate(() => {
      ...
      for (const script of scripts) {
        ...
        const voeRedirectMatch = content.match(
    ...
    if (!extractedMediaUrl) {
      return res
        .status(404)
        .json({ error: "Direct video stream URL could not be extracted." });
    }
